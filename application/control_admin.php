...
class control_admin extends CI_Controller
{
    ...
    public function jsonSales(){
        ...
        foreach($fetch_data as $row){
            ...
            $sub_array[] = '<a class="btn btn-success pull-right btn-xs" id="salesModal" data-toggle="modal" data-target="#modal-lg"
                            ...
                            <a href="'.base_url().'index.php/control_admin/edit_sales/'.$row->id_sales.'" class="btn btn-secondary pull-right btn-xs" style="padding-left: 10px; padding-right: 10px;" role="button"><i class="fa fa-align-justify"></i> Edit</a>
    ...
    public function jsonApproval(){
        ...
        foreach($fetch_data as $row){
            ...
            $sub_array[] = $row->booking_order; 
            $sub_array[] = $row->pic_sales;           
            $sub_array[] = '<a href="'.base_url().'index.php/control_admin/'.$row->id_sales.'" class="btn btn-success pull-right btn-xs" role="button"><i class="fa fa-download"></i> Approval</a>
            <a href="'.base_url().'index.php/control_admin/edit_sales/'.$row->id_sales.'" class="btn btn-secondary pull-right btn-xs" style="padding-left: 10px; padding-right: 10px;" role="button"><i class="fa fa-align-justify"></i> Edit</a>
            <a href="'.base_url().'index.php/control_admin/delete_sales/'.$row->id_sales.'" class="btn btn-danger pull-right btn-xs" role="button"><i class="fa fa-trash"></i> Hapus</a>'; 
    ...
    public function getAddSales()
    {
        ...
        $pic_customer   = $this->input->post('pcus');
        ...
        $order_day      = $this->input->post('wkts');
        $toll_parkir    = $this->input->post('toll');
        $revenue_unit   = $this->input->post('rvnu');
        $addcharge      = $this->input->post('addc');
        $addcharge_ket  = $this->input->post('ket_addc');
        $revenue_total  = $this->input->post('total_rvnu');
