...
class control_sales extends CI_Controller{
    ...
    {
        $data['sales'] = $this->m_sales->getSales_all_data()->result();
        $this->load->view('template/header');
        $this->load->view('template/sidebar');
        $this->load->view('sales/v_sales', $data);
        $this->load->view('template/footer');
    }

    public function jsonSales()
    {
        $fetch_data = $this->m_sales->useDatatables();
        $data = array();
        $no = $_POST['start'];
        foreach ($fetch_data as $row) {
            ...
            $sub_array[] = '<a href="'.base_url().'index.php/control_sales/add_sales_more/'.$row->id.'" class="btn btn-primary pull-right btn-xs" role="button" style="padding-left: 5px; padding-right: 5px;"><i class="fas fa-plus"></i></a>
            <a class="btn btn-primary pull-right btn-xs" id="salesModal" data-toggle="modal" data-target="#modal-lg"
                            ...
                            <a href="'.base_url().'index.php/control_sales/edit_sales/'.$row->id.'" class="btn btn-warning pull-right btn-xs" style="padding-left: 10px; padding-right: 10px;" role="button"><i class="fas fa-edit"></i> <b>Edit</b></a>
        ...
        $output = array(
            ...
            "recordsTotal" => $this->m_sales->get_all_datatables(),
            "recordsFiltered" => $this->m_sales->get_filtered_datatables(),
            "data" => $data
        );
    ...
    {
        if (isset($_GET['term'])) {
            $result = $this->m_sales->getUnit_all_data($_GET['term']);
            if (count($result) > 0) {
    ...
    public function add_sales()
    {
        ...
        $this->load->view('sales/v_add_sales', $data);
    }

    public function add_sales_more($id)
    {
        $where = array('id' => $id);
        $data = $this->m_sales->getSelect_data();
        $data['fetch_data'] = $this->m_sales->useSales_bookOrd($where, 'sales')->result();
        $this->load->view("sales/v_addmore_sales", $data);
    }

    public function getAddSales()
    {
        $rules = $this->m_sales->validationForm();
        ...
        $data2 = array(
            'pay_1'     => $postData['py1'],
            'pay_tgl_1' => date("Y-m-d", strtotime($postData['tgl_py1'])),
            'pay_nom_1' => $postData['nom_py1'],                            
            'pay_2'     => $postData['py2'],
            'pay_tgl_2' => date("Y-m-d", strtotime($postData['tgl_py2'])),  
            'pay_nom_2' => $postData['nom_py2'],
            'pay_3'     => $postData['py3'],            
            'pay_tgl_3' => date("Y-m-d", strtotime($postData['tgl_py3'])),
            'pay_nom_3' => $postData['nom_py3'],         
            'pay_4'     => $postData['py4'],
            'pay_tgl_4' => date("Y-m-d", strtotime($postData['tgl_py4'])),         
            'pay_nom_4' => $postData['nom_py4'],
            'pay_5'     => $postData['py5'],            
            'pay_tgl_5' => date("Y-m-d", strtotime($postData['tgl_py5'])),
            'pay_nom_5' => $postData['nom_py5'],             
        );

        $id_payment = $this->m_sales->insert_data('sales_payment', $data2);
        ...
        $this->m_sales->insert_data('sales', $data);
